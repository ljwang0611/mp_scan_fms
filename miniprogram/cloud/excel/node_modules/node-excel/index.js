...
exports.execute = function (config) {
	var headers = config.headers[0];
	var dataHeaders = headers.data;
	...
	if (headers) {
		customHeight = "";
		if (headers.height) {
			customHeight = ' ht="' + headers.height + '" customHeight="1"';
		}
		row = '<x:row r="1" spans="1:' + colsLength + '"' + customHeight + '>';
		var headerCaption, headerStyleIndex;
		for (k = 0; k < colsLength; k++) {
			headerCaption = "";
			if (dataHeaders[k]) {
				headerCaption = (dataHeaders[k].caption || '');
			}
			headerStyleIndex = (dataCols[k].styleHeaderIndex || 0);
			row += addStringCol(getColumnLetter(k + 1) + 1, headerCaption, headerStyleIndex);
		}
		row += '</x:row>';
		rows += row;
	}	
	...
	customHeight = "";
	if (cols.height) {
		customHeight = ' ht="' + cols.height + '" customHeight="1"';
	}
	row = '<x:row r="2" spans="1:' + colsLength + '"'+ customHeight + '>';
	for (k = 0; k < colsLength; k++) {
		colStyleIndex = dataCols[k].styleColIndex || 0;
		row += addStringCol(getColumnLetter(k + 1) + 2, dataCols[k].caption, colStyleIndex);
		if(headers === undefined){
			if (dataCols[k].width) {
				colsWidth += '<col customWidth="1" width="' + dataCols[k].width + '" max="' + (k + 1) + '" min="' + (k + 1) + '"/>';
			}
		}
	}
	row += '</x:row>';
